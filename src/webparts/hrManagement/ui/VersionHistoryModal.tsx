...
const VersionHistoryModal: React.FC<VersionHistoryModalProps> = ({
  ...
  const body = (
    <div className="version-history-modal">
      {isLoading && <div className="text-muted px-2 py-2">Loading version history...</div>}
      {!isLoading && error && <div className="alert alert-danger py-2 px-3 mb-0">{error}</div>}
      {!isLoading && !error && entries.length === 0 && <div className="text-muted px-2 py-2">No version history found for this item.</div>}
