...
export function SmartTable<T extends { id: string }>(props: SmartTableProps<T>) {
  ...
  return (
    <div className={`smart-table-wrapper h-100 d-flex flex-column ${!props.tableSettings.showHeader ? 'no-header' : ''}`}>
      {props.tableSettings.showHeader && (
        <TableToolbar
          totalCount={props.totalCount ?? props.data.length}
          ...
          showAdvancedSearch={props.tableSettings.showAdvancedSearch}
        />
      )}
      <DataTable
        data={props.data}
        ...
        showColumnFilter={props.tableSettings.showColumnFilter}
      />
